...
function setSelectedStop(from, to) {
  ...
  fromStopInput.dispatchEvent(new Event('input'));
  toStopInput.dispatchEvent(new Event('input'));
...
async function openTicketViewPage(payment = false) {
  if (payment === true) {
    ...
    const rawDetails = localStorage.getItem("tempTicketDetails");
    const ticketDetail = rawDetails ? JSON.parse(rawDetails) : {};
    ticketDetail.userId = userId;
    ticketDetail.price = parseInt(localStorage.getItem("price")) || 0;
    ticketDetail.ticketPrice = ticketDetail.price * parseInt(localStorage.getItem("totalTickets"));
    ...
    let tickets = rawTickets ? JSON.parse(rawTickets) : [];
...
fromStopInput.addEventListener("click", () => {
  window.location.href = "./stopSelection.html";
});
