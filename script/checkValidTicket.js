document.addEventListener("DOMContentLoaded", function() {
  ...
  function checkTicketValidityOnLoad() {
    ...
    if (ticketData) {
      ...
      if (remainingTime > 0) {
        ...
        window.location = "./cityBusBooking.html";
        // "http://127.0.0.1:5500";
      }
    } else {
  ...
  function crateResentOrderList(limit=0) {
    ...
    if (!pastTicketsData) {
      console.log("No past tickets found.");
      return;
    }
    const tickets = (JSON.parse(pastTicketsData) || []);
    ...
    if (tickets.length > 0 && recentOrderWrapper && recentOrderTempalate) {
      ...
      for (let i = 0; i < tickets.length; i++) {
        ...
        recentOrderTempalateClone.querySelector('.past-order-prise').innerText = ticket.ticketPrice || ticket.ticketPrise;
